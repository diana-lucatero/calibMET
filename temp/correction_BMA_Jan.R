...
for (ngrid in 1:1){ # Loop for space :dim(grid)[1]
  ...
  for (nyear in 0:(dimIndex[1]-1) ){  #  (dimIndex[1]-1) Loop for year of validation
    ...
    for (nt in 1:7){#7
      ...
      train = t(matrix(Xpredictor0,nrow = no_ens,ncol=(dimIndex[1]-1)*d[nt]))
      ens_data = ensembleData(forecasts=train,observations=Ypredictand0,verbose=FALSE,dates=NULL,forecastHour = NULL,initializationTime = NULL)
      calib = fitBMAnormal(ens_data,exchangeable = exc)
      for (nlead in s0[nt]:s1[nt]){
        ...
        Probcurve = 1 - cdf.fitBMAnormal(calib, fore, values = thres)

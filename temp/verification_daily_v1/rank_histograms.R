...
for (imeth in 1:length(meth)){#length(meth)
  for (lt in (1:7)){
    name_plt = paste(ObsDir,'verification_daily/',meth[imeth],'_',quality,'_lt',lt,'.png',sep="")
    png(name_plt,width=750,height=750)
    layout(mat = m)#, widths=c(2,2,2,0.5), heights=array(1,dim=c(4)))
    ## Start plotting
    for (nmonth in 1:Nmonth){#Nmonth
      name_file = paste(ObsDir,'verification_daily/',meth[imeth],'_',quality,'_m',nmonth,'_lt',lt,'.txt',sep="")

...
for (nmonth in 1:1){#Nmonth
  for (lt in (1:Nlead)){#Nlead
    print(lt)
    values0 = array(NaN,dim=c(length(meth),Ngrids,(Nens[nmonth]+1)))
    values1 = array(NaN,dim=c(length(meth),Ngrids,Nsam[nmonth]))
    for (imeth in 1:length(meth)){#length(meth)
      ## Rank histogram
      name_file = paste(ObsDir,'verification_weekly/',meth[imeth],'_RH_m',nmonth,'_lt',lt,'.txt',sep="")
      val = t(matrix(scan(name_file),nrow=(Nens[nmonth]+1),ncol=Ngrids))
      values0[imeth,,] = val
      ## Sharpness
      name_file = paste(ObsDir,'verification_weekly/',meth[imeth],'_sharp_m',nmonth,'_lt',lt,'.txt',sep="")
      val = t(matrix(scan(name_file),nrow=Nsam[nmonth],ncol=Ngrids))
      values1[imeth,,] = val
    } # End methods read
    ## Plot
    for (ngrid in (1:Ngrids)){#662

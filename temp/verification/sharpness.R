...
for (imeth in 1:length(meth)){#length(meth)
  for (lt in (1:7)){
    for (nmonth in 1:Nmonth){#Nmonth
      ...
      val = t(matrix(scan(name_file),nrow=Nsam[nmonth],ncol=724))
...
#par(cex.lab=3)
layout(mat = m, widths=c(2,2,2,2,0.5), heights=array(1,dim=c(4)))
nHalf = 11
...
ColorLevels = seq(0, 6, length=length(ColorRamp))
...
for (imet in 1:4){
  ...
  main0 = meth[imet]
  #par(mar = c(3,2,2.5,1))
  image(t(dat),col=ColorRamp , ylab="", axes=FALSE, zlim=c(0,6),main = main0,xlab = 'lt (months)')

...
for (ngrid in 1:dim(grid)[1]){ # Loop for space :dim(grid)[1]
  ...
  Pobs_m = cbind(rowMeans(obs[,1:30],na.rm = T),rowMeans(obs[,31:60],na.rm = T),rowMeans(obs[,61:90],na.rm = T),rowMeans(obs[,91:120],na.rm = T),rowMeans(obs[,121:150],na.rm = T),rowMeans(obs[,151:180],na.rm = T),rowMeans(obs[,181:210],na.rm = T))
  rm(obs)
  ## Climatology
...
# Save verification 724(ngrid) x 7(nlead)
# Raw

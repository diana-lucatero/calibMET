...
for (ngrid in 42:length(grid)){ # Loop for space :length(grid)
  print(ngrid)
  ens_filename = paste(Dir,'ensembles_idw/ens_station_',grid[ngrid,2],'.txt',sep='')
  ...
  rm(ens)
  rkEns = cbind(nams,Pens_r);rm(Pens_r)
  ## Save ranks
  name_file = paste(Dir,'ensembles_r_idw/ens_station_',grid[ngrid,2],'.txt',sep='')

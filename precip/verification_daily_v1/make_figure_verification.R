...
values = array(NaN,dim=c(length(meth),662,215))
for (imeth in 1:length(meth)){#length(meth)
      ...
      val = t(matrix(scan(name_file),nrow=215,ncol=662))
      values[imeth,,] = val
} # End method
...
for (lt in 1:Nlead) { # Start lead time
  ...
  name_plt = paste(ObsDir,'verification_daily/plots/',quality[iq],'_m',nmonth,'_lt',lt,'.png',sep='')

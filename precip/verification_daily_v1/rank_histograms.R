...
m <- matrix(seq(1:6),nrow = 2,ncol = 3,byrow = FALSE) # No Meth + 1 (for legend)
...
for (nmonth in 1:1){#Nmonth
  for (lt in (1:1)){#Nlead
    ...
    for (imeth in 1:length(meth)){#length(meth)
      ## Rank histogram
      name_file = paste(ObsDir,'verification_daily/',meth[imeth],'_RH_m',nmonth,'_lt',lt,'.txt',sep="")
      val = t(matrix(scan(name_file),nrow=(Nens[nmonth]+1),ncol=662))
      values0[imeth,,] = val
      ## Sharpness
      name_file = paste(ObsDir,'verification_daily/',meth[imeth],'_sharp_m',nmonth,'_lt',lt,'.txt',sep="")
      val = t(matrix(scan(name_file),nrow=Nsam[nmonth],ncol=662))
      values1[imeth,,] = val
    } # End methods read
    ## Plot
    for (ngrid in (1:3)){#662
      name_plt = paste(ObsDir,'verification_daily/plots/g',ngrid,'_lt',lt,'.png',sep="")
      ...
      for (imet in 1:3){
        ...
        barplot(values0[imet,ngrid,],names.arg = arg0,main=main0,ylim  = c(0,25))
        hist(values1[imet,ngrid,],main = '',xlab = '',ylab = '',ylim  = c(0,25))
      } # End methods plot
      graphics.off()
    } # End grid plot
  } # End lead time

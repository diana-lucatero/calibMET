...
ObsDir = 'F:/ECMWF_Seasonal_data/calibMET'
...
for (ngrid in 1:dim(grid)[1]){ # Loop for space :dim(grid)[1]
  ...
  ens0 = t(matrix(scan(ens_filename0),nrow=217,ncol=8322));ens0[ens0<1.5]=0;
...
write(t(crpssr_clim),file=paste(ObsDir,'/verification_daily/ver_B_',nmonth,'.txt',sep=''),ncolumns=1)
write(t(len),file=paste(ObsDir,'/verification_daily/verL_B_',nmonth,'.txt',sep=''),ncolumns=1)

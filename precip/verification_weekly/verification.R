...
source(paste0(ObsDir,'/verification_weekly/upscale.R'))
meth = c('bma') #meth = c('dc','qm','bma')
Nlead = 30 # weeks
month = seq(1,12,1);mlead = seq(1,Nlead,1)
...
for (ngrid in 1:1){ # Loop for space :dim(grid)[1]
  ...
  for (nmet in 1:length(meth)){#length(meth)
    ...
    if (find.file(ens_namefile) == ''){ ens = array(NaN,dim = c(dimIndex[1]*no_ens,215)) } 
    else { ens = t(matrix(scan(ens_namefile),nrow=215,ncol=dimIndex[1]*no_ens)) }
...
# Save verification 662(ngrid) x Nlead(nlead)
# Raw
# write(t(biasr),file=paste(ObsDir,'/verification_weekly/R_bias_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
# write(t(corrr),file=paste(ObsDir,'/verification_weekly/R_corr_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
# write(t(crpsr),file=paste(ObsDir,'/verification_weekly/R_crps_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
# write(t(crpssr_clim),file=paste(ObsDir,'/verification_weekly/R_crpss_clim_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
# for (il in 1:Nlead) {
#   write(t(rh00[,il,]),file=paste(ObsDir,'/verification_weekly/R_RH_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=(no_ens+1))
#   write(t(sharp00[,il,]),file=paste(ObsDir,'/verification_weekly/R_sharp_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=dimIndex)
# }
# # Methods
# for (imeth in 1:length(meth)){
#   write(t(biasm[,,imeth]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_bias_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(corrm[,,imeth]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_corr_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(crpsm[,,imeth]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],':crps_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(crpssm_raw[,,imeth]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_crpss_raw_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(crpssm_clim[,,imeth]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_crpss_clim_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   for (il in 1:Nlead) {
#     write(t(rh[,il,imeth,]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_RH_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=(no_ens+1))
#     write(t(sharp[,il,imeth,]),file=paste(ObsDir,'/verification_weekly/',meth[imeth],'_sharp_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=dimIndex)
#   }
# }

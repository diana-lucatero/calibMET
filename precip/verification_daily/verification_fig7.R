...
## Upscale to 20 km
name_file <- paste0(ObsDir,'/verification_daily/precip_20.txt')
squares <- read.table(name_file)
# Read grid points in squares
Pclim = array(NaN, dim=c(dimIndex[1],215))
# Verification arrays raw
crpssr_clim = array(NaN,dim=c(70,Nup))

for (nsqr in 1:70){ # Loop for space :70
  print(nsqr)
  ...
  for (ngrid in 1:Ngrid){
    ## Observations
    obs_namefile = paste(ObsDir,'/observation/obs_station_',igrd[grid[ngrid,1],grid[ngrid,2]],'.txt',sep='')
    obs = t(matrix(scan(obs_namefile),nrow=216,ncol=293));obs[obs<0]=0;
...
## Upscale to 40 km
name_file <- paste0(ObsDir,'/verification_daily/precip_40.txt')
squares <- read.table(name_file)
# Read grid points in squares
Pclim = array(NaN, dim=c(dimIndex[1],215))
# Verification arrays raw
crpssr_clim = array(NaN,dim=c(24,Nup))

for (nsqr in 1:24){ # Loop for space :24
  print(nsqr)

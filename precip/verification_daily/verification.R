...
for (ngrid in 1:1){ # Loop for space :dim(grid)[1]
  ...
  for (nyear in 1:dimIndex[1]){
    Pclim[nyear,] = colMeans(obs[-(nyear),1:Nlead])
  }
  ## Ensembles Raw
...
# write(t(corrr),file=paste(ObsDir,'/verification_daily/R_corr_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
# write(t(crpsr),file=paste(ObsDir,'/verification_daily/R_crps_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
...
#   write(t(sharp00[,il,]),file=paste(ObsDir,'/verification_daily/R_sharp_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=dimIndex)
# }
# # Methods
# for (imeth in 1:2){
#   write(t(biasm[,,imeth]),file=paste(ObsDir,'/verification_daily/',meth[imeth],'_bias_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(corrm[,,imeth]),file=paste(ObsDir,'/verification_daily/',meth[imeth],'_corr_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(crpsm[,,imeth]),file=paste(ObsDir,'/verification_daily/',meth[imeth],':crps_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
#   write(t(crpssm_raw[,,imeth]),file=paste(ObsDir,'/verification_daily/',meth[imeth],'_crpss_raw_m',nmonth,'.txt',sep=''),ncolumns=Nlead)
...
#     write(t(sharp[,il,imeth,]),file=paste(ObsDir,'/verification_daily/',meth[imeth],'_sharp_m',nmonth,'_lt',il,'.txt',sep=''),ncolumns=dimIndex)
#   }
# }

...
#   }
  ...
  for (nmet in 1:length(meth)){#length(meth)
    ...
    Pens_met    = cbind(rowSums(ens[,1:30],na.rm = TRUE),rowSums(ens[,31:60],na.rm = TRUE),rowSums(ens[,61:90],na.rm = TRUE),rowSums(ens[,91:120],na.rm = TRUE),rowSums(ens[,121:150],na.rm = TRUE),rowSums(ens[,151:180],na.rm = TRUE),rowSums(ens[,181:210],na.rm = TRUE))
    rm(ens)

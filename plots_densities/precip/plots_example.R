...
ens0 = Pens[-(index_nyear),nlead];d=ecdf(ens0) # train ens
obs0 =Pobs[-(nyear+1),nlead];d1=ecdf(obs0) # train obs
...
par(mar=mar0)
main0 = 'LS'
plot(d1,main = main0,lty = 5,xlim = ylim0) # observation train
...
ecdf1 = lapply(1:no_ens, function(iens) ecdf(ens00[iens,])) # ecdf for each ens
par(mar=mar0)
main0 = 'QM'
plot(d1,main = main0,lty = 5,xlim = ylim0) # observation train
for (iens in 1:no_ens){
  plot(ecdf1[[iens]],add = TRUE,col = 'azure3')
}
points(x1,y0,col='blue') # before ens
points(x0,y0,col='red') # final ens
abline(v = Pobs[(nyear+1),nlead],col='maroon1') # observation validate
...
points(x1,y0,col='blue') # before ens
points(x0,y0,col='red') # final ens
points(thres,(1-Probcurve),col = 'green4',pch = 16)
abline(v = Pobs[(nyear+1),nlead],col='maroon1') # observation validate
## Boxplot
par(mar=mar1)
data <- data.frame(raw = x1, corr = x0)
